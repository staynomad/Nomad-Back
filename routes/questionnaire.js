...
router.post("/submit_questionnaire/:id", async (req, res) => {
  // formatting req body
  const {
    name,
    numberOfRoommates,
  ...
  const prevQuestionnaire = await Questionnaire.findOne({
  ...
  if (prevQuestionnaire) {
    Questionnaire.findOneAndUpdate(
      ...
      .then(() =>
        console.log("success in updating previous questionnaire response!")
      )
  ...
  // if user is submitting a questionnaire for the first time
    ...
    try {
      ...
      res.status(500).json({
        errors: [
          "Error occurred while submitting questionnaire. Please try again!",
        ],
      });
    }
  }
